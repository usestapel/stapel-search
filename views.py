...
_QUERY_PARAMS = [
    ...
    OpenApiParameter("bands", str, description="on | off. On, the answer is ordered in two labelled bands — inside `near_radius_km` of lat/lon first, then everything else. Nothing is ever withheld by distance. Default: GEO_BANDS."),
    OpenApiParameter("near_radius_km", float, description="The near band's EDGE, around the lat/lon centre. Never a filter — a row outside it is labelled `far`, not dropped. Use `radius_km` when you mean to exclude. Default: NEAR_BAND_RADIUS_KM."),
